...
class CMSC_Helper
{
    ...
    {		
        ...
        if ($md5 === false) {
        	...
        	update_option('cmsc_log_md5', $md5);
        }
	...
	function cmsc_parse_action_params( $key = '', $params = null, $call_object = null ){
		
		global $_cmsc_item_filter;
		$call_object = $call_object !== null ? $call_object : $this;
		$return = array();
		
		if(isset($_cmsc_item_filter[$key]) && !empty($_cmsc_item_filter[$key])){
			if( isset($params['item_filter']) && !empty($params['item_filter'])){
				foreach($params['item_filter'] as $_items){
					if(!empty($_items)){
						foreach($_items as $_item){
							if(isset($_item[0]) && in_array($_item[0], $_cmsc_item_filter[$key])){
	...
	function cmsc_set_transient($option_name = false, $data = false){
		
		if (!$option_name || !$data) {
            return false;
        }
		if($this->cmsc_multisite)
			return $this->cmsc_set_sitemeta_transient($option_name, $data);
    ...
    function cmsc_get_transient($option_name)
    {
        if (trim($option_name) == '') {
            return FALSE;
        }
	...
	function cmsc_get_sitemeta_transient($option_name){
		...
		$result = $wpdb->get_var( $wpdb->prepare("SELECT `meta_value` FROM `{$wpdb->sitemeta}` WHERE meta_key = %s AND `site_id` = %s", $option_name, $this->cmsc_multisite)); 
		$result = maybe_unserialize($result);
		return $result;
	}
	
	function cmsc_set_sitemeta_transient($option_name, $option_value){
		global $wpdb;
		$option_name = '_site_transient_'. $option_name;
		
		if($this->cmsc_get_sitemeta_transient($option_name)){
    ...
    function set_cmsc_message_id($message_id = false)
    {
        if ($message_id) {
            add_option('_cmsc_action_message_id', $message_id) or update_option('_cmsc_action_message_id', $message_id);
            return $message_id;
        }
        return false;
    }
    ...
    {
        return (int) get_option('_cmsc_action_message_id');
    }
    
    function set_master_public_key($public_key = false)
    {
        if ($public_key && !get_option('_cmsc_public_key')) {
            add_option('_cmsc_public_key', base64_encode($public_key));
            return true;
        }
        return false;
    }
    ...
    function get_master_public_key()
    {
        if (!get_option('_cmsc_public_key'))
            return false;
        return base64_decode(get_option('_cmsc_public_key'));
    }
    ...
    {
        if ($random_key && !get_option('_cmsc_nossl_key')) {
            add_option('_cmsc_nossl_key', base64_encode($random_key));
            return true;
        }
        return false;
    }
    ...
    function authenticate_message($data = false, $signature = false, $message_id = false)
    {
        ...
        $current_message = 0;// $this->get_worker_message_id();
        
        if ((int) $current_message >= (int) $message_id)
            return array(
                'error' => 'Invalid message recieved. Deactivate and activate the CMS Commander plugin on this site, then re-add it to your CMS Commander account and add it again.'
            );
        
        $pl_key = $this->get_master_public_key();
        if (!$pl_key) {
            return array(
                'error' => 'Authentication failed. Deactivate and activate the CMS Commander plugin on this site, then re-add it to your CMS Commander account and add it again.'
            );
        }	
        ...
        if (function_exists('openssl_verify') && !$this->get_random_signature()) {
            ...
            if ($verify == 1) {
                $message_id = $this->set_cmsc_message_id($message_id);
                return true;
            } else if ($verify == 0) {
                return array(
                    'error' => 'Invalid message signature. Deactivate and activate the CMS Commander plugin on this site, then re-add it to your CMS Commander account and add it again.'
                );
            } else {
                return array(
        ...
        // no rand key - deleted in get_stat maybe
            ...
            return array(
                'error' => 'Invalid message signature. Deactivate and activate the CMS Commander plugin on this site, then re-add it to from your CMS Commander account and add it again.'
            );
    }
    ...
    function check_if_user_exists($username = false)
    {
        ...
        if ($username) {
            ...
            $user = (array) $this->cmsc_get_user_info( $username );
    ...
    function cmsc_get_error($error_object)
    {
