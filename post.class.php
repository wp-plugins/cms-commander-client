...
class CMSC_Post extends CMSC_Core
{
	...
	function get_posts($args){
		...
		{
			$where.=" AND post_date BETWEEN '".mysql_real_escape_string($cmsc_get_posts_date_from)."' AND '".mysql_real_escape_string($cmsc_get_posts_date_to)."'";
		}
		else if(!empty($cmsc_get_posts_date_from) && empty($cmsc_get_posts_date_to))
		{
			$where.=" AND post_date >= '".mysql_real_escape_string($cmsc_get_posts_date_from)."'";
		}
		else if(empty($cmsc_get_posts_date_from) && !empty($cmsc_get_posts_date_to))
		{
			$where.=" AND post_date <= '".mysql_real_escape_string($cmsc_get_posts_date_to)."'";
		}
		$post_array=array();
	...
	function get_pages($args){
		...
		{
			$where.=" AND post_date BETWEEN '".mysql_real_escape_string($cmsc_get_pages_date_from)."' AND '".mysql_real_escape_string($cmsc_get_pages_date_to)."'";
		}
		else if(!empty($cmsc_get_pages_date_from) && empty($cmsc_get_pages_date_to))
		{
			$where.=" AND post_date >= '".mysql_real_escape_string($cmsc_get_pages_date_from)."'";
		}
		else if(empty($cmsc_get_pages_date_from) && !empty($cmsc_get_pages_date_to))
		{
			$where.=" AND post_date <= '".mysql_real_escape_string($cmsc_get_pages_date_to)."'";
		}
		...
		// CMSC Change - Search custom post types
		if(!empty($search_custom)) {$posttype = $search_custom;} else {$posttype = "page";}
		
		$sql_query = "$wpdb->posts  WHERE post_status!='auto-draft' AND post_status!='inherit' AND post_type='".$posttype."' ".$where.' ORDER BY post_date DESC';
